...
| Three wrong turns on the way, recorded so they are not taken twice | (1) A settle-then-read change to the check: the fit was settled, at the wrong place. (2) Ad-hoc probes appeared to show a theme switch dropping the suggested bus; a no-switch control collapsed at the same moment; the cause was the probe context letting the service worker install and bypass `page.route`. (3) A transient-resize theory, and a re-fit-after-resize handler built on it: the spec's screenshot is a viewport one and resizes nothing, and the handler was not the fix. It was first kept as harmless; the next full suite showed it **fighting the ride's exit** — leaving the ride calls `resize()` on purpose, and the handler's second fit, carrying no camera, froze the tilt at 14.2° mid-glide in three checks on both projects. Removed. Speculative behaviour added on a wrong diagnosis does not get to stay because it seems benign | — | Removed |

## 8b. The release (21 September 2026, evening)

| | |
|---|---|
| Deployed commit | `509ae40`; served page byte-identical to the local build; previous release kept for rollback |
| Whole suite on that build, one run | **230 passed, 24 skipped by design, 0 failed** (35.2 min). Earlier full runs on the way: 225/24/5 (stale caption, restated), 224/24/6 (caption by mode: fixed), 228/24/2 (camera competition: fixed), 224/24/6 (resize handler fighting the exit: removed) — none of them was clean and none is presented as such |
| Node, Python, typecheck, lint | 194 / 131 / clean / 0 errors, same commit |
| Served | 363 shape files, 362-pattern index (114 accepted), the 263 shape; collector, health, refresh, arrival units active |
| Nightly rebuild | Fired unattended, Monday 02:42 UTC — the open item from 20 September is closed |
| Coverage at deploy, served publication | 650 vehicles: road/front view 105 (16%), estimated 15 (2%), travelling between reports 635 (98%) |
